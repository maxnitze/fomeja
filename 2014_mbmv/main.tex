...
\begin{figure}[t!]
\begin{minted}[fontsize=\footnotesize,bgcolor=bg,linenos=true]{java}
public class Vertex {
  private int id;
...
\mintinline{java}|@Condition| annotiert und beschreiben die Bedingungen für eine
...
durch `\mintinline{java}|...|' ausgelassenen Zeilen beschreiben Methoden zum
Aufbauen des Graphen).  In der \mintinline{java}|main| Methode wird eine Instanz
des Graphen erzeugt und eine Datei eingelesen. Danach ist der Graph zunächst
ungefärbt.  Der Aufruf von \mintinline{java}|DIAB.satisfy(g)| belegt alle mit
\mintinline{java}|@Variable| annotierten Feldvariablen, die über die
Graphinstanz erreicht werden können, so dass alle Methoden die als
\mintinline{java}|@Constraint| annotiert sind, erfüllt werden.
...
\mintinline{java}|edges| erreicht werden.  Die Constraint Methoden werden
...
\mintinline{java}|@Variable| Annotation ermittelt werden.  Alle Formeln werden
...
\mintinline{java}|@Variable| annotierten Feldvariablen ausgelesen werden.
...
Hier wird die Methode \mintinline{java}|numColors| als Optimierungskriterium
durch die Annotation \mintinline{java}|@Objective| gekennzeichnet.  In der
